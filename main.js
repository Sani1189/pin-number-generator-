...
document.getElementById('btn-back').addEventListener('click', function () {
    const num = document.getElementById('match-pin').value;
    const digits = num.split('');
    digits.pop();
    const remainingDigits = digits.join('');
    document.getElementById('match-pin').value = remainingDigits;

    btncheck();
});
function btncheck() {
    let num = document.getElementById('match-pin').value;
    let str = ""+num;
    if (str.length >= 4 && tr > 0) {
        btnEnable();
    }
    else{
        btnDisable();
